...
```
### On command handling, without passing the complete client:
Using Discord.js v13 slash commands.
...
module.exports = {
    ...
    async execute(interaction) {
        ...
        if (interaction.member.voice.channel) {
            discordTogether.createTogetherCode(interaction.member.voice.channel.id, 'youtube')
                .then(res => { return interaction.reply(`${invite.code}`) });
        }
